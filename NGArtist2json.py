...
def NGAudioArtistHtml2json(NGAudioArtistHtml):
	...
	audiojson: list = [{}]
	audiojson[0]["artistNickname"
	            ] = artistNickname = NGAudioArtistHtmlRoot.cssselect(
	...
	NGAudioArtistHtmlRootSelectionMatches = NGAudioArtistHtmlRoot.cssselect(
	    '.audio-wrapper'
	)
	...
	for i, audioMatch in enumerate(NGAudioArtistHtmlRootSelectionMatches):
		...
		audioMatchUrl = NGAudioArtistHtml.xpath(audioMatchPath +
		                                        "/div/a")[0].get("href")
		# print("audioMatchUrl: ", audioMatchUrl)
		audioMatchID = re.match(
		    r".+(?<=listen/)([^\'\"\s.\\/]+.*)", audioMatchUrl
		).group(1)
		...
		audioMatchIDFields["title"] = audioMatch.cssselect(".detail-title")[
		    0].text_content().strip()
		audioMatchIDFields["description"] = audioMatch.cssselect(
		    ".detail-description"
		)[0].text_content().strip()
		AudioYearPath = re.sub(
		    r"/[^/]+/[^/]+/[^/]+/[^/]+$", "", audioMatchPath, count=1
		)
		...
		audioMatchIDFields["NGScore"] = audioMatch.cssselect(
		    "div.item-details-meta > div.star-score"
		...
		audioMatchIDFields["NGViews"] = int(
		    audioMatch.cssselect(
		        "div.item-details > div.item-details-meta > dl > dd:nth-child(3)"
...
def main() -> None:
	parser = argparse.ArgumentParser(
	    description="Tool to extract metadata from Newgrounds audio artist pages."
	)
	parser.add_argument(
	    "-jl",
	    "-json-file-list",
	...
	for NGAudioArtistHtmlFile in NGAudioArtistHtmlFilelist:
		...
		writeToFile(
			...
			orjson.dumps(audiojson, option=orjson.OPT_INDENT_2).decode('utf-8'),
			openmode='w',
			file_encoding='utf-8'
		)
		if args.jl:
