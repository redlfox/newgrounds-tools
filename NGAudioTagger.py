...
def NGAudioTagEditor(
    mp3FilePath: Path,
    NGJSONData: dict = None,
    KeepValidYear: bool = True,
    KeepValidTitle: bool = True,
    KeepValidArtist: bool = True,
    KeepNGAPAlbum: bool = True,
    KeepOrigComment: bool = True,
	...
	try:
		...
		print(
			f"Can't read ID3v2 tag from file. Error: {e}"
		)
	...
	try:
		NGID = int(
		    re.match(r".+(((?<=sub\=)|(?<=listen/))\d+).*",
		             Id3fileCommentText).group(1)
		)
	...
	if NGID and NGJSONData:
		...
		try:
			for artistInfo in NGJSONData:
				...
				if Id3fileArtistText.casefold(
				) == artistInfo["artistNickname"].casefold():
			...
			if artistMatched:
				...
				for artistInfo in NGJSONData:
					for audioId in artistInfo["musicPublished"]:
						...
						if NGID == int(audioData["NGId"]):
							print(
							    "Matched audio ID without artist match: ", NGID
							)
							artistMatched = artistInfo
							audioItemMatched = audioData
							break
			...
			if audioItemMatched:
				# Fill tags here
				if re.match(
				    r"\s*", Id3fileTitleText
				) or not Id3fileTitleText or not KeepValidTitle:
					...
					Id3file["TIT2"] = TIT2(
					    encoding=Id3fileTitleEncoding,
					    text=audioItemMatched["title"] + " (ID: " + str(NGID) +
					    ")"
					)
				if re.match(
				    r"\s*", Id3fileArtistText
				) or not Id3fileArtistText or not KeepValidArtist:
					...
					Id3file["TPE1"] = TPE1(
					    encoding=Id3fileArtistEncoding,
					    text=artistMatched["artistNickname"]
					)
				if not 2003 <= int(
				    Id3fileYearv23Text
				) < 2100 or not Id3fileYearv23Text or not KeepValidYear:
					print("Filling year tag")
					Id3file["TYER"] = TYER(
					    encoding=3, text=str(audioItemMatched["yearPublished"])
					)
				if not audioItemMatched[
				    "description"
				] in Id3fileDescriptionText or not Id3fileDescriptionText:
					print("Filling description tag")
					Id3file["TXXX"] = TXXX(
					    encoding=3,
					    desc="DESCRIPTION",
					    text=audioItemMatched["description"]
					)
					Id3file.save(v2_version=3)
					Id3file = ID3(mp3FilePath, v2_version=3)
				Id3file["TXXX"] = TXXX(
				    encoding=3, desc="NGTYPE", text=audioItemMatched["type"]
				)
				if re.match(
				    r"\s*", Id3fileCommentText
				) or not Id3fileCommentText or not KeepOrigComment:
					...
					Id3file["COMM"] = COMM(
					    encoding=Id3fileCommentEncoding,
					    lang="eng",
					    desc="",
					    text="This track can be found at: " +
					    audioItemMatched["url"]
					)
				Id3file.save(v2_version=3)
				Id3file = ID3(mp3FilePath, v2_version=3)
				Id3file["TCON"] = TCON(
				    encoding=3, text=audioItemMatched["genre"]
				)
				Id3file["TXXX"] = TXXX(
				    encoding=3,
				    desc="NGSCORE",
				    text=audioItemMatched["NGScore"]
				)
				Id3file.save(v2_version=3)
				Id3file = ID3(mp3FilePath, v2_version=3)
				Id3file["TXXX"] = TXXX(
				    encoding=3,
				    desc="NGVIEWS",
				    text=str(audioItemMatched["NGViews"])
				)
				Id3file.save(v2_version=3)
				Id3file = ID3(mp3FilePath, v2_version=3)
				if not Id3fileAlbumText or not KeepNGAPAlbum:
					...
					Id3file["TALB"] = TALB(
					    encoding=Id3fileAlbumEncoding, text=albumName
					)
				fileEdited = True
				# sys.exit()
				...
				print(
				    "No audio item match found for ID ", NGID, " in NGJSONData"
				)
				fileFailedtoEditTags = mp3FilePath
		except Exception as e:
	...
	if Id3file.get("TIT2"):
		...
		if re.match(r".+\(ID: \d+\).*", Id3fileTitleText):
			...
			if NGID:
				...
				Id3file["TIT2"] = TIT2(
					encoding=Id3file["TIT2"].encoding,
					text=Id3fileTitleText + " (ID: " + str(NGID) + ")"
				)
				Id3file.save(v2_version=3)
				print(
					"Edited file's title metadata to contain ID and saved: ",
					mp3FilePath
				)
			else:
				print("No ID match found in comment for ", mp3FilePath)
...
def NGAudioRenamer(mp3FilePath: Path,aliasesJson:list):
	...
	if aliasesJson:
		for artistInfo in aliasesJson:
			...
			if artistInfoAlias:
				...
				continue
	artistValidFilename = re.sub("\\/","-",re.sub(
	    r"[:*?\"<>|]", "", Id3file["TPE1"].text[0]
	))
	...
	if PurePath(mp3FilePath).stem != re.sub(r"\.mp3$", "", mp3FileExceptName):
		print(
		    "File name does not match metadata title and artist for ",
		    mp3FilePath
		)
		newMp3FilePath: Path = Path(
		    PurePath(mp3FilePath).parent / mp3FileExceptName
		)
...
def main() -> None:
	...
	parser = argparse.ArgumentParser(
	    description="Tool to edit Newgrounds audio's tags."
	)
	parser.add_argument(
	    "-f",
	    "-fuzzy-match",
	...
	parser.add_argument(
	    ...
	    default=None,
	    help=
	    "JSON file that storage artists' alias names."
	)
	parser.add_argument(
	    "-ngapl",
	    default=None,
	...
	for countIndex, NGAudioArtistHtmlFileItem in enumerate(
	    NGAudioArtistHtmlFilelist, start=1
	):
		if Path(NGAudioArtistHtmlFileItem).is_file() is False:
			print("File does not exist: ", NGAudioArtistHtmlFileItem)
			continue
		with open(
		    NGAudioArtistHtmlFileItem,
		    "r",
		    encoding="utf-8"
		) as f:
			try:
		...
		if countIndex == 1:
			print(
			    "Extending NGJSONData with data from ",
			    NGAudioArtistHtmlFileItem
			)
	...
	print(
	    "Files failed to edit tags: ", filesFailedtoEditTags,
	    "\nFiles failed to rename: ", filesFailedtoRename
	)
	# NGAudioTagEditor(mp3FilePath, NGJSONData)
	# NGAudioRenamer(mp3FilePath)
